...
// state == STATE_SEND means: bus empty, packet can sent to destination
                    slices++;
                    try {
                    ...
                    if (slices > SLOT_TIME / 50) {
                        System.out.println("host[" + id + "] send the msg without collisions!");
...
// state == STATE_COLLISION means::collision betweenpackets
                    slices = 0;
// increase idle time after each failed transmission

                    try {
                        collisions++;
                        BackOffTimer backofftimer = new

                                BackOffTimer(collisions);
